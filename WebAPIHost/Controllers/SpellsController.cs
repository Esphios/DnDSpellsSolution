...
public class SpellsController(ISpellRepository spellRepository) : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            (IEnumerable<Spell>? Spells, int TotalItems, int CurrentPage) = await _spellRepository.GetAllSpellsAsync(
    ...
    {
        Spell? spell = await _spellRepository.GetSpellByIdAsync(id, cancellationToken);
        return spell == null ? NotFound() : Ok(spell);
    }
